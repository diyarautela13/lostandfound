...
async function start(user) {
  ...
  if (!claimId) {
    headerDiv.textContent = "No chat was specified. Go back to Messages and click Chat on a claim.";
  ...
  let claimSnap;
  try {
    claimSnap = await getDoc(doc(db, "claims", claimId));
  } catch (err) {
    console.error(err);
    headerDiv.textContent = "Could not load this chat (permission error). See console.";
    form.style.display = "none";
    return;
  }
  ...
  const otherUserId = user.uid === claim.finderId ? claim.claimantId : claim.finderId;
